...
func main() {
	...
	if err = db.AutoMigrate(&model.User{}, &model.Repair{}, &model.Payment{}, &model.Announcement{}, &model.AnnouncementRead{}, &model.OperationLog{}, &model.Role{}, &model.Permission{}, &model.RolePermission{}, &model.VisitorPass{}); err != nil {
		log.Fatal(err)
	}
	...
	vr := repository.NewVisitorPassRepository(db)
	lr := repository.NewOperationLogRepository(db)
	sv := router.Services{Users: service.NewUserService(ur, logger), Repairs: service.NewRepairService(rr, ur, logger), Payments: service.NewPaymentService(pr, logger), Announcements: service.NewAnnouncementService(ar, logger), VisitorPasses: service.NewVisitorPassService(vr, logger), Permissions: service.NewPermissionService(), Logs: service.NewOperationLogService(lr, logger)}
...
func seed(db *gorm.DB) error {
	...
	for _, p := range []model.Permission{{Code: "repair:manage", Name: "报修管理"}, {Code: "payment:manage", Name: "收费管理"}, {Code: "announcement:publish", Name: "公告发布"}, {Code: "log:read", Name: "日志查看"}, {Code: "visitor:approve", Name: "访客审批"}} {
		if e = db.Create(&p).Error; e != nil {
			return e
		}
	}
	demoStart := time.Now().Add(2 * time.Hour).Truncate(time.Minute)
	if e = db.Create(&model.VisitorPass{UserID: users[0].ID, Plate: "粤B8A888", NormalizedPlate: "粤B8A888", VisitorName: "李师傅", Building: users[0].Building, Room: users[0].Room, StartAt: demoStart, EndAt: demoStart.Add(3 * time.Hour), Status: constants.VisitorStatusPending}).Error; e != nil {
		return e
	}
	fmt.Print("")
	return nil
