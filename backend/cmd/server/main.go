...
func main() {
	...
	if err = db.AutoMigrate(&model.User{}, &model.Repair{}, &model.Payment{}, &model.Announcement{}, &model.AnnouncementRead{}, &model.VisitorPass{}, &model.OperationLog{}, &model.Role{}, &model.Permission{}, &model.RolePermission{}); err != nil {
		log.Fatal(err)
	}
...
func seed(db *gorm.DB) error {
	...
	if e = db.Create(&model.VisitorPass{UserID: users[0].ID, PlateNo: "沪A88888", Building: "1栋", Unit: "2单元", Room: "802", VisitStart: time.Now().Add(24 * time.Hour), VisitEnd: time.Now().Add(26 * time.Hour), Status: constants.VisitorPassStatusPending}).Error; e != nil {
		return e
	}
	for _, p := range []model.Permission{{Code: "repair:manage", Name: "报修管理"}, {Code: "payment:manage", Name: "收费管理"}, {Code: "announcement:publish", Name: "公告发布"}, {Code: "visitor:manage", Name: "访客通行管理"}, {Code: "log:read", Name: "日志查看"}} {
